  ...
  return (
    <div className="min-w-0 space-y-4">
      ...
      <div className="bg-[var(--ty-fill-white-color)] border border-[var(--ty-border-color)] rounded-ty-sm p-4 flex flex-wrap items-end gap-3">
        <label className="text-ty-xs text-[var(--ty-font-sub-color)] space-y-1"><span className="block">对象类型</span><select aria-label="排查对象类型" value={root} onChange={(event) => { setRoot(event.target.value); setSelected(null); setFilter('ALL'); }} className="h-8 min-w-40 px-3 border border-[var(--ty-border-color)] rounded-ty-sm bg-[var(--ty-fill-white-color)] text-[var(--ty-font-main-color)]">{roots.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
