    ...
    <div className="min-w-0 space-y-4">
      <header className="bg-[var(--ty-fill-white-color)] border border-[var(--ty-border-color)] rounded-ty-sm p-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2"><Database className="w-5 h-5 text-[var(--ty-primary-color)]" /><h1 className="text-ty-md font-semibold">目标端多余数据排查</h1><span className="text-ty-2xs px-2 py-0.5 rounded-ty-xs bg-[var(--ty-fill-color)] text-[var(--ty-font-sub-color)] border border-[var(--ty-border-color)]">Manticore → 源端反查</span></div>
          <p className="text-ty-xs text-[var(--ty-font-sub-color)] mt-1">按对象标识反查源端存在性，在证据完整后确认多余数据。</p>
        </div>
        <button onClick={() => { setExample(!example); setSelected(null); setFilter('ALL'); }} className="h-8 px-3 border border-[var(--ty-border-color)] rounded-ty-sm text-ty-xs hover:bg-[var(--ty-fill-weak-dark-color)]">{example ? '返回实际数据' : '查看判断示例'}</button>
      </header>
      ...
      <div className="bg-[var(--ty-fill-white-color)] border border-[var(--ty-border-color)] rounded-ty-sm p-3.5 flex flex-wrap items-end gap-3">
        <label className="text-ty-xs text-[var(--ty-font-sub-color)] space-y-1"><span className="block">对象类型</span><select aria-label="排查对象类型" value={root} onChange={(event) => { setRoot(event.target.value); setSelected(null); setFilter('ALL'); }} className="h-8 min-w-40 px-2.5 border border-[var(--ty-border-color)] rounded-ty-sm bg-[var(--ty-fill-white-color)] text-[var(--ty-font-main-color)]">{roots.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        {example && <label className="text-ty-xs text-[var(--ty-font-sub-color)] space-y-1"><span className="block">示例场景</span><select aria-label="排查示例场景" value={scenario} onChange={(event) => { setScenario(event.target.value); setSelected(null); }} className="h-8 min-w-52 px-2.5 border border-[var(--ty-border-color)] rounded-ty-sm bg-[var(--ty-fill-white-color)] text-[var(--ty-font-main-color)]"><option value="COMPLETE">完整查询：确认多余</option><option value="INCOMPLETE">源端查询不完整</option><option value="FORBIDDEN">源端权限不足</option><option value="ERROR">源端读取失败</option></select></label>}
        <button disabled title="源端与目标端存在性查询未接入" className="h-8 px-3 rounded-ty-sm text-ty-xs bg-[var(--ty-primary-lighter-color)] text-[var(--ty-font-sub-light-color)] cursor-not-allowed inline-flex items-center gap-1.5"><Search className="w-3.5 h-3.5" />开始排查</button>
      ...
      ].map(([label, count]) => <div key={String(label)} className="bg-[var(--ty-fill-white-color)] border border-[var(--ty-border-color)] rounded-ty-sm p-3.5"><span className="text-ty-xs text-[var(--ty-font-sub-color)] block">{label}</span><strong className="text-ty-xl mt-1 block">{count ?? '待获取'}</strong></div>)}</div>

      <section className="bg-[var(--ty-fill-white-color)] border border-[var(--ty-border-color)] rounded-ty-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-[var(--ty-border-color)] flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-ty-sm font-semibold">排查明细</h2><p className="text-ty-xs text-[var(--ty-font-sub-color)] mt-1">{snapshot ? `快照 ${snapshot.snapshotId} · ${snapshot.capturedAt}` : '存在性查询未接入，暂无可确认结果。'}</p></div><label className="text-ty-xs text-[var(--ty-font-sub-color)] space-y-1"><span className="block">排查结果</span><select aria-label="排查结果" value={filter} onChange={(event) => { setFilter(event.target.value); setSelected(null); }} className="h-8 min-w-40 px-2.5 border border-[var(--ty-border-color)] rounded-ty-sm bg-[var(--ty-fill-white-color)] text-[var(--ty-font-main-color)]"><option value="ALL">全部</option><option value="EXTRA">确认多余</option><option value="UNKNOWN">待确认 / 读取异常</option><option value="PRESENT">源端存在</option></select></label></div>
        {visible.length ? <div className="overflow-x-auto"><table className="w-full text-left text-ty-xs border-collapse"><thead><tr className="bg-[var(--ty-fill-color)] text-[var(--ty-font-sub-color)]"><th className="px-4 py-2.5 font-medium">目标对象标识</th><th className="px-4 py-2.5 font-medium">目标记录 ID</th><th className="px-4 py-2.5 font-medium">源端查询</th><th className="px-4 py-2.5 font-medium">结论</th><th className="px-4 py-2.5 font-medium">操作</th></tr></thead><tbody>{visible.map((row) => <tr key={row.targetId} className="border-t border-[var(--ty-border-light-color)]"><td className="px-4 py-3 font-mono">{row.objectId}</td><td className="px-4 py-3 font-mono">{row.targetId}</td><td className="px-4 py-3">{{ FOUND: '已找到', NOT_FOUND: '未找到', FORBIDDEN: '权限不足', ERROR: '读取失败' }[row.sourceResult]}</td><td className="px-4 py-3"><span className={`inline-flex px-2 py-0.5 rounded-ty-xs border ${badge(row.status)}`}>{row.label}</span></td><td className="px-4 py-3"><button className="text-[var(--ty-primary-color)]" onClick={() => setSelected(row.targetId)}>查看依据</button></td></tr>)}</tbody></table></div> : <div className="p-10 text-center text-ty-xs text-[var(--ty-font-sub-color)]">{snapshot ? '当前条件下暂无记录' : '等待接入源端与目标端存在性查询'}</div>}
        {detail && <div className="m-4 p-4 bg-[var(--ty-fill-weak-dark-color)] border border-[var(--ty-border-color)] rounded-ty-sm text-ty-xs space-y-2"><div className="flex items-center justify-between"><h3 className="font-semibold">{detail.objectId} · 排查依据</h3><button className="text-[var(--ty-primary-color)]" onClick={() => setSelected(null)}>收起</button></div><p>{detail.reason}</p><p className="text-[var(--ty-font-sub-color)]">{detail.note || '继续核查唯一标识、查询范围和同步时点。'}</p><ol className="list-decimal pl-5 space-y-1"><li>确认源端对象是否删除、失效或移出同步范围。</li><li>分别检查源端采集日志和 Manticore 同步记录。</li><li>检查唯一标识变化或重复写入，修复后重新核验。</li></ol>{!example && <button className="text-[var(--ty-primary-color)]" onClick={() => onSync(root)}>查看该类型同步记录</button>}</div>}
      </section>
    </div>
  );
}
