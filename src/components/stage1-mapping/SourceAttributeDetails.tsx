...
export function SourceAttributeDetails({ meta, compact = false }: { meta: SourceFieldMeta; compact?: boolean }) {
  ...
  return (
    <div className={`source-attribute-details${compact ? ' source-attribute-details--compact' : ''}`}>
      ...
      {compact ? <>
        <p><span className="source-attribute-label">来源表：</span>{tables}</p>
        <p>{kind} · {meta.isMultiValue === undefined ? '多值状态未返回' : meta.isMultiValue ? '多值' : '单值'}</p>
        {!enumDefined && <p>枚举定义：{enumDefined === undefined ? '未返回' : '无'}</p>}
      </> : <dl>
