...
import { getSimilarityRuleScopeLabel } from '../data';
...
export const PublishRecordView: React.FC<PublishRecordViewProps> = ({ changeRecords }) => {
  ...
  const scopeOptions = Array.from(new Set(changeRecords.map(record => record.groupValueId).filter(Boolean))) as string[];
  ...
  return <div className="space-y-4" id="publish-record-view-container">
    <header className="bg-[var(--ty-fill-white-color)] rounded-ty-sm border border-[var(--ty-border-color)] px-4 py-3 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2"><History className="w-5 h-5 text-[var(--ty-primary-color)]" /><h1 className="text-ty-xl font-semibold">配置变更审计历史</h1><HelpTooltip label="查看配置变更审计历史说明" content="追溯零部件各类型通用或分类专用规则范围的保存、发布、启用、停用、失败原因及变更前后摘要，包括分档阈值修改。" /></div>
      </div>
    ...
    <section className="bg-[var(--ty-fill-white-color)] border border-[var(--ty-border-color)] rounded-ty-sm p-3 flex flex-wrap items-end gap-3">
      <label className="text-ty-xs text-[var(--ty-font-sub-color)] space-y-1"><span className="block">规则范围</span><select value={groupValue} onChange={event => { setGroupValue(event.target.value); setPage(1); }} className="h-8 min-w-52 px-3 border border-[var(--ty-border-color)] rounded-ty-sm bg-white"><option value="ALL">全部规则范围</option>{scopeOptions.map(scopeKey => <option key={scopeKey} value={scopeKey}>{getSimilarityRuleScopeLabel(scopeKey)}</option>)}</select></label>
      <label className="text-ty-xs text-[var(--ty-font-sub-color)] space-y-1"><span className="block">操作类型</span><select value={operation} onChange={event => { setOperation(event.target.value); setPage(1); }} className="h-8 min-w-32 px-3 border border-[var(--ty-border-color)] rounded-ty-sm bg-white"><option value="ALL">全部操作</option><option value="保存">保存</option><option value="发布">发布</option><option value="启用">启用</option><option value="停用">停用</option></select></label>
