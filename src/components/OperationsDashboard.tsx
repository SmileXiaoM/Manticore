...
export function OperationsDashboard({ roots, checks, ingestionLogs, syncRecords, schedules, onIngestion, onSync, onCheck, onPresence }: {
  ...
  return <div className="min-w-0 space-y-4">
    <header className="bg-white border border-[var(--ty-border-color)] rounded-ty-sm px-4 py-3 flex flex-wrap items-center justify-between gap-3">
    ...
    <section className="bg-white border border-[var(--ty-border-color)] rounded-ty-sm p-3 flex flex-wrap items-center gap-4">
      <label className="text-ty-xs text-[var(--ty-font-sub-color)] flex items-center gap-2">对象类型<select aria-label="看板对象类型" value={filter} onChange={(event) => { setFilter(event.target.value); setPage(1); }} className="h-8 min-w-40 px-3 border border-[var(--ty-border-color)] rounded-ty-sm bg-white text-[var(--ty-font-main-color)]"><option value="ALL">全部类型</option>{roots.map((root) => <option key={root.id} value={root.id}>{root.name}</option>)}</select></label>
      <label className="text-ty-xs text-[var(--ty-font-sub-color)] flex items-center gap-2">统计周期<select aria-label="看板统计周期" value={period} onChange={(event) => setPeriod(event.target.value as StatisticPeriod)} className="h-8 min-w-36 px-3 border border-[var(--ty-border-color)] rounded-ty-sm bg-white text-[var(--ty-font-main-color)]">{Object.entries(periodMeta).map(([value, meta]) => <option key={value} value={value}>{meta.label}</option>)}</select></label>
      <span className="text-ty-2xs text-[var(--ty-font-sub-color)] ml-auto">大数字按万/亿缩写，悬停可查看精确值</span>
    </section>

    <section className="grid grid-cols-2 xl:grid-cols-4 gap-3">{cards.map((card) => <button key={card.label} onClick={card.action} className="bg-white border border-[var(--ty-border-color)] rounded-ty-sm p-4 flex items-center justify-between text-left hover:border-[var(--ty-primary-color)]"><div className="min-w-0"><span className="text-ty-xs text-[var(--ty-font-sub-color)] block">{card.label}</span><strong className="text-ty-xl mt-1 block font-mono" title={card.exactValue}>{card.value}</strong><small className="text-ty-2xs text-[var(--ty-font-sub-color)] mt-1 block">{card.note}</small></div><div className="w-9 h-9 shrink-0 rounded-ty-sm bg-[var(--ty-fill-color)] text-[var(--ty-font-sub-color)] flex items-center justify-center">{card.icon}</div></button>)}</section>

    <section className="bg-white border border-[var(--ty-border-color)] rounded-ty-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-[var(--ty-border-color)]"><h2 className="text-ty-sm font-semibold">各类型运行情况</h2><p className="text-ty-xs text-[var(--ty-font-sub-color)] mt-1">写入数据按统计周期汇总；同步队列突出当前待处理与失败记录。</p></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-ty-xs"><thead className="bg-[var(--ty-fill-weak-dark-color)] text-[var(--ty-font-sub-color)]"><tr><th className="px-3 py-2 text-left">对象类型</th><th className="px-3 py-2 text-left">接入状态 / 检查频率</th><th className="px-3 py-2 text-left">中间表写入（{periodMeta[period].shortLabel}）</th><th className="px-3 py-2 text-left">Manticore 同步</th><th className="px-3 py-2 text-left">最近核验</th><th className="px-3 py-2 text-left">目标数据</th></tr></thead><tbody className="divide-y divide-[var(--ty-border-light-color)]">{pageRoots.map((root) => {
        ...
        return <tr key={root.id} className="hover:bg-[var(--ty-fill-weak-dark-color)]/50">
          ...
          <td className="px-3 py-3"><strong className="font-mono" title={(root.manticoreDocCount || 0).toLocaleString()}>{formatCompactNumber(root.manticoreDocCount || 0)}</strong><span className="block mt-1 text-[var(--ty-font-sub-color)]">Manticore 记录</span><button className="text-[var(--ty-primary-color)] mt-2 inline-flex items-center gap-1" onClick={() => onPresence(root.id)}>多余数据排查<ArrowUpRight className="w-3 h-3" /></button></td>
        </tr>;
      })}</tbody></table></div>
