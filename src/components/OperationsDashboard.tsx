...
export function OperationsDashboard({ roots, checks, ingestionLogs, syncRecords, schedules, onIngestion, onSync, onCheck, onPresence }: {
  ...
  const targetBacklog = targetRows.filter((row) => row.status === 'PENDING').length;
  ...
  const cards = [
    { label: '中间表写入日志', value: sourceRows.length, note: `成功 ${sourceRows.filter((r) => r.status === 'SUCCESS').length} · 失败 ${sourceRows.filter((r) => r.status === 'FAILED').length}`, icon: <ScrollText className="w-4 h-4" />, action: () => onIngestion(selected) },
    { label: 'Manticore 同步队列', value: targetBacklog, note: `中间表 ${targetRows.length} · 已处理 ${targetRows.filter((r) => r.status === 'PROCESSED').length} · 失败 ${targetRows.filter((r) => r.status === 'FAILED').length}`, icon: <Server className="w-4 h-4" />, action: () => onSync(selected) },
  ...
  return <div className="min-w-0 space-y-4">
    <header className="bg-white border border-[var(--ty-border-color)] rounded-ty-sm px-4 py-3 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><FileCheck2 className="w-5 h-5 text-[var(--ty-primary-color)]" /><div><div className="flex items-center gap-2"><h1 className="text-ty-xl font-semibold">Manticore 运行看板</h1><span className="text-ty-2xs min-h-6 px-2 inline-flex items-center rounded-ty-xs bg-[var(--ty-fill-color)] text-[var(--ty-font-sub-color)] border border-[var(--ty-border-color)]">多类型总览</span></div><p className="text-ty-xs text-[var(--ty-font-sub-color)] mt-1">统一查看中间表写入日志、Manticore 同步队列、一致性核验与目标数据现状。</p></div></div><span className="min-h-6 px-2 inline-flex items-center rounded-ty-xs bg-[var(--ty-green-lightest-color)] text-[var(--ty-green-color)] border border-[var(--ty-green-color)]/30 text-ty-xs"><Server className="w-3.5 h-3.5 mr-1" />同步服务运行中</span></header>
